/**
 * Unified-portal request proxy (renamed from the deprecated
 * `middleware.ts` convention — Next.js 16 ships a `proxy.ts`
 * replacement with an identical request/response shape).
 *
 ...
 *   - `/`, `/about`, `/services/**`, `/contact`, `/blog`, `/faq`, `/assessment`,
 *     and the legal pages are PUBLIC — no auth check at all.
 *
...
export async function proxy(req: NextRequest) {
  ...
  if (token && isPublicAuth(pathname)) {
    return NextResponse.redirect(
      new URL(landingForRole(role ?? "DOCTOR"), req.url),
    )
  }
...
// Match everything except static + API. The proxy itself filters further.
export const config = {
