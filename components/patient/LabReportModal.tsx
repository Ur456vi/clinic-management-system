 ...
 *
 * A test can hold MULTIPLE report files. This modal lists every file the
 * patient has uploaded for the order, lets them add more (multi-select, PDF,
 * 25 MB each), view any one, and delete individual files.
 *
 *   - list   GET    /api/patient/me/lab-results/:id/reports
 *   - add    POST   /api/patient/me/lab-results/:id/report        (multipart)
 *   - view   GET    /api/patient/me/lab-results/:id/reports/:fileId
 *   - delete DELETE /api/patient/me/lab-results/:id/reports/:fileId
 */
...
export default function LabReportModal({
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      const res = await fetch(`/api/patient/me/lab-results/${labResultId}/reports`, {
        credentials: "include",
      });
      ...
      setError("Couldn't load report files.");
      setFiles([]);
    }
  }, [labResultId]);
  ...
  const onPick = useCallback(
    async (picked: FileList | null) => {
      ...
      setUploading(false);
      if (failures > 0) {
        setError(`${failures} file${failures === 1 ? "" : "s"} couldn't be uploaded (PDF only, max 25 MB).`);
      }
      await load();
      onUploaded();
    },
    [labResultId, uploading, load, onUploaded],
  );
  ...
  const viewFile = useCallback(
    async (fileId: string) => {
      setError(null);
      try {
        const res = await fetch(`/api/patient/me/lab-results/${labResultId}/reports/${fileId}`, {
          credentials: "include",
        });
        if (!res.ok) throw new Error();
        const json = await res.json();
